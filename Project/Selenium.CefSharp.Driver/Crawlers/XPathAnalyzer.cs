...
namespace Selenium.CefSharp.Driver
{
    ...
    public static class XPathAnalyzer
    {
        ...
        private static double PM25 = 1.4;
            ...
            bool isAttrEnabled)
        {
            ...
            if (node1.ChildNodes.Count == 1 && node1.ChildNodes[0].NodeType == HtmlNodeType.Text)
            {
                ...
                {
                    var name = node1.SearchPropertyName(result) ?? "attribute" + result.Count;
            ...
            foreach (var attribute in node1.Attributes)
            {
                ...
                var row = nodes.Select(d =>
                {
                    ...
                    {
                        Console.WriteLine($"XPath expression is written in error: {ex.Message}");
                        return null;
                    }
                })
                    .Where(d => d != null)
            ...
            if (nodes.Any(d => d.Name.Contains("#")))
            {
                ...
                if (nodes.Any(d => d.InnerText != first.InnerText))
                {
                    ...
                    if (buffers.Any(d => ListEqual(d, row)))
                        return isChildContainInfo;
                    var name = "attribute" + result.Count;
        ...
        public static IEnumerable<HtmlNode> SelectNodesPlus(this HtmlNode node, string xpath, SelectorFormat format)
        {
            ...
            {
                Console.WriteLine($"The path parsing is incorrect and returns to the empty node:{xpath}");
                return null;
            }
        }
        ...
        public static void GetDataFromXPath(this HtmlNode doc, CrawlItem crawItem, string root,
            FreeDocument document, SelectorFormat rootFormat = SelectorFormat.XPath)
        {
        ...
        /// <param name="crawItem"></param>
        /// <param name="document"></param>
        public static void GetDataFromXPath(this HtmlNode doc, CrawlItem crawItem, FreeDocument document)
        {
            ...
            IEnumerable<CrawlItem> exists = null, int minNodeCount = 2)
        {
            ...
            {
                Console.WriteLine($"Maybe the XPath expression is wrong: {ex.Message}");
            ...
            SelectorFormat format = SelectorFormat.XPath, bool isAttrEnabled = false)
        {
            ...
            foreach (var existItem in existItems)
            {
                ...
                if (items.Count() == 1)
                {
                    ...
                    var count = 0;
                    var target =
                        getCrawTarget(xpaths.Select(d => new CrawlItem { XPath = d, Name = ("attribute" + "_" + count++) }).ToList(), "")
                        ;
                    target.RootNode = doc2;
            ...
            SelectorFormat rootFormat = SelectorFormat.XPath)
        {
            ...
            switch (type)
            {
                case ScriptWorkMode.List:
                    ...
                    {
                        if (string.IsNullOrEmpty(rootXPath))
                            throw new Exception("When the extraction mode is List and the selector is CssSelecor, the root node path must be set.");
